          ...
          if(_stricmp(lo->day, myday) != 0)
            {
              ...
              n = lp_this->buidByte(it1->first, 0x4, 0xAC, 0x71, 0, 0x310, v_b, activepower);
              lp_this->InitIoContext(lo);
  ...
  while(TRUE)
    {
      ...
      if(lp_io == NULL)
        {
          // lp_this->PostLog("GetQueuedCompletionStatus lp_io:%p bRet:%d ThreadId:%d ErrorCode:%d", lp_io, bRet, GetCurrentThreadId(), WSAGetLastError());
          glog::traceErrorInfo("\nGetQueuedCompletionStatus lp_io is NULL  GetQueuedCompletionStatus", WSAGetLastError());
      ...
      if(FALSE == bRet && GetLastError() != 0)
        {
          glog::GetInstance()->AddLine("通信出错:%p fromtype:%d 错误代码:%d", lp_io, lp_io->fromtype, WSAGetLastError());
          ...
          lp_this->PostLog("GetQueuedCompletionStatus ErrorCode:%d  lp_io:%p   from:%d ", WSAGetLastError(), lp_io,  lp_io->fromtype);
          //EnterCriticalSection(&lp_this->crtc_sec);
          lp_this->m_io_group.RemoveAt(lp_io);
          lp_this->m_key_group.RemoveAt(lp_key);
                  ...
                  if(itepack == lp_this->m_pack.end())
                    {
                      ...
                      if(bBreakPack)
                        {
                          ...
                          glog::GetInstance()->AddLine("断包包头:lp_io:%p", lp_io);
#endif
                        }
                      ...
                      //            b->len = lp_io->ol.InternalHigh;
                      //            lp_this->m_pack.insert(make_pair(lp_io, b));
                      //            lp_this->PostLog("断包包尾:lp_io:%p", lp_io);
                      //          }
                      //      }
                      //  }
                    }
                  else
                    {
                      ...
                      if(lp_io->fromtype == SOCKET_FROM_Concentrator)
                        {
                          lp_this->PostLog("断包包尾:lp_io:%p", lp_io);
                          glog::GetInstance()->AddLine("断包包尾:lp_io:%p", lp_io);
                        }
          ...
          if(DA[0] == 0 && DA[1] == 0 && DT[0] == 0x01 && DT[1] == 0x05)
            {
              ...
              sql = "select * from t_power where 1=1 and CONVERT(Nvarchar, day, 23)=\'";
              sql.append(myday);
              sql.append("\' and comaddr='");
              sql.append(addrarea);
              sql.append("'");
              ...
              if(rs && this->dbopen.GetNum(rs) == 0)
                {
                  sql = "insert into t_power(day,comaddr,power00,power45) values(\'";
                  sql.append(myday);
                  sql.append("\',\'");
                  sql.append(addrarea);
                  sql.append("\',\'");
                ...
                {
                  sql = "update t_power set power00=\'";
                  sql.append(power00);
                  sql.append("\',power45=\'");
                  sql.append(power45);
                  sql.append("\'");
              ...
              sql = "UPDATE t_power SET POWER45=\'";
              sql.append(power00);
              sql.append("\'");
