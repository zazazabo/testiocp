                      ...
                      if(len > 0)
                        {
                          ...
                          if(ite != m_mcontralcenter.end())
                            {
                              BYTE seq = 0;
                              map<string, vector<MSGPACK>>::iterator itmsg = m_MsgPack.find(addrarea);

                              if(itmsg != m_MsgPack.end())
                                {
                                  vector<MSGPACK>v_msg = itmsg->second;
                                  seq = v_msg[v_msg.size() - 1].seq;
                                }

                              seq = itmsg == m_MsgPack.end() ? 0 : seq + 1;
                              ...
                              if(msgType != "00")
                                {
                                  ...
                                  if(itmsg == m_MsgPack.end())
                                    {
                                      vector<_MSGPACK>v_msgpack;
                                      m_MsgPack.insert(pair<string, vector<MSGPACK>>(addrarea, v_msgpack));
                                    }

                                  itmsg = m_MsgPack.find(addrarea);
                                  itmsg->second.push_back(msg);
                                  int ncount = itmsg->second.size();
                                  PostLog("网关[%d] 消息长度[%d] 末帧序列[%d]", itmsg->first.c_str(), itmsg->second.size(), seq);
                              ...
                              PostLog("转换后:%s 长度:%d", tosenddata.c_str(), tosenddata.size());
  ...
  while(TRUE)
    {
      ...
      if((IOCP_ACCEPT != lp_io->operation) && (0 == dwBytes))
        {
          lp_this->ExitSocket(lp_io, lp_key);
          continue;
        }
      ...
      if(DirPrmCode == 0x80 && con == 0x0 && FC == 0x8)   //  上行 从动 响应帧   0x80 上行 从动
        {
          ...
          if(itmsg != m_MsgPack.end())
            {
              vector<MSGPACK>v_msg = itmsg->second;
              vector<MSGPACK>::reverse_iterator it = v_msg.rbegin();
          ...
          //if(!m_listmsg.empty())
          //  {
          //    _MSGPACK msg = m_listmsg.back();
          //    if(msg.seq != frame || _stricmp(addrarea, msg.comaddr) != 0)
          //      {
          //        glog::GetInstance()->AddLine("帧序列域错误 集中器响应帧:%d 消息列表:%d", frame, msg.seq);
          //        // m_listmsg.clear();
          //      }
          //    else if(msg.seq == frame && _stricmp(addrarea, msg.comaddr) == 0)
          //      {
          //        IOCP_IO_PTR lp_io1 = msg.lp_io;
          //        m_listmsg.pop_back();
          //        string strret = "";
          //        BOOL bFullPack = FALSE;
          //        int lenread = wsDecodeFrame(lp_io1->buf, strret, lp_io1->ol.InternalHigh, bFullPack);
          ...
          //    }
          //}
          //else if(DA == 0 && DA[1] == 0 && DT[0] == 2 && DT[1] == 0)   //全部否认
          //{
          //    //全部否认
          //    if(!m_listmsg.empty())
          //    {
          ...
          //    }
          //}
          //else if(DA == 0 && DA[1] == 0 && DT[0] == 4 && DT[1] == 0)    //按单元标识事件确认
          //{
          //    if(!m_listmsg.empty())
          //    {
          //        IOCP_IO_PTR lp_io1 = m_listmsg.back();
...
void CIOCP::ExitSocket(IOCP_IO_PTR& lp_io, IOCP_KEY_PTR& lp_key)
{
  string towrite = "";
  EnterCriticalSection(&crtc_sec);
  ...
  if(lp_io->fromtype == SOCKET_FROM_Concentrator)
    {
      ...
      map<string, vector<MSGPACK>>::iterator itMsg = m_MsgPack.find(comaddr);
  ...
  if(lp_io->fromtype == SOCKET_FROM_WEBSOCKET)
    {
      map<string, vector<MSGPACK>>::iterator it = m_MsgPack.begin();

      while(it != m_MsgPack.end())
        {
          vector<MSGPACK>v_msg = it->second;
  ...
  m_io_group.RemoveAt(lp_io);
  m_key_group.RemoveAt(lp_key);
  n11 = m_io_group.GetCount();
  n00 = m_io_group.GetBlankCount();
  PostLog("ExitSocket  lp_io:%p  List1 count:%d List2 count:%d 客户端类型:%d", lp_io, n11, n00, lp_io->fromtype);
}
