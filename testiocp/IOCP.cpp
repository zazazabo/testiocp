        ...
        if(msgType.isString())
        {
            ...
            if(tosend.isString() && tosend != "")
            {
                ...
                else if(msgType == "ALL")
                {
                    ...
                    for(auto it = m_mcontralcenter.begin(); it != m_mcontralcenter.end();)
                    {
                        setOnline(it->first, 1);
                        it++;
                    }
                }
            }
...
//  gstring::split(v_str,m_configTime,":");
    ...
    while(TRUE)
    {
        ...
        if(lp_this->dbopen->isConectDb() == FALSE)
        {
            glog::GetInstance()->AddLine("db close");
            ...
            BOOL bcon = lp_this->dbopen->ConnToDB(source, database, uname, upass);
        }
...
BOOL CIOCP::InitAll()
{
	...
	//{
	//	0x68, 0x5E, 0x00, 0x5E, 0x00, 0x68, 0xC4, 0x02, 0x17, 0x01, 0x01, 0x04, 0x0E, 0x62, 0x00, 0x00,
	//	0x01, 0x00, 0x2C, 0x09, 0x47, 0x14, 0x15, 0x10, 0x18, 0x36, 0x00, 0x01, 0x00, 0x58, 0x16
	//};
	//IOCP_IO_PTR p1;
