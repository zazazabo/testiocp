    ...
    else if(lenread != WS_ERROR_FRAME)
    {
        ...
        if(reader.parse(strret.c_str(), root))
        {
            ...
            if(msgType.isString())
            {
                ...
                //if(msgType == "AA" || msgType == "A4" || msgType == "A5" || msgType == "AC")
                //{
                ...
                //        glog::GetInstance()->AddLine("总消息长度:%d 消息:%s ", m_listmsg.size(), root["data"].asString().c_str());
                //        PostLog("总消息长度:%d 消息:%s ", m_listmsg.size(), root["data"].asString().c_str());
                //    }
                //}
...
DWORD CIOCP::CompletionRoutine(LPVOID lp_param)
{
    ...
    while(TRUE)
    {
        ...
        if((IOCP_ACCEPT != lp_io->operation) && (0 == dwBytes))
        {
            ...
            lp_this->PostLog("CompletionRoutine  lp_io:%p  List1 count:%d List2 count:%d 客户端类型:%d", lp_io, n11, n00, lp_io->fromtype);
            continue;
        }
    ...
    else if(AFN == 0x00 || AFN == 0xAA || AFN == 0xA4)      //全部确认
    {
        if(DirPrmCode == 0x80 && con == 0x0 && FC == 0x8)   //  上行 从动 响应帧   0x80 上行 从动
        {
            if(!m_listmsg.empty())
            {
                ...
                if(msg.seq != frame || stricmp(addrarea, msg.comaddr) != 0)
                {
