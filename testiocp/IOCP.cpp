            ...
            case IOCP_READ:
                {
                    ...
                    {
#ifdef _DEBUG
                        glog::GetInstance()->AddLine("其它数据包 长度:%d:数据:%s", lp_io->ol.InternalHigh, towrite.c_str());
#endif
                        //集中器断包处理
                        ...
                        else
                        {
                            ...
                            if(bBreakPack == FALSE)
                            {
                                BOOL bret =  lp_this->IsTailPack((BYTE*)lp_io->buf, lp_io->ol.InternalHigh, itepack->second, lp_io);
