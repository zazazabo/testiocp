...
DWORD CIOCP::CompletionRoutine(LPVOID lp_param)
{
    ...
    while(TRUE)
    {
        ...
        if(FALSE == bRet && lp_io != NULL)
        {
            ...
            lp_this->DeleteByIo((ULONG_PTR)lp_io);
            ...
            case IOCP_READ:
                {
                    ...
                    towrite = data;
                    int ncount = lp_this->m_plistuser->GetCount();

                    for(int i = 0; i < ncount; i++)
                    {
                        string iostr = lp_this->getItemText(lp_this->m_plistuser, i, 2);
                        char pio[20] = {0};
                        sprintf(pio, "%08x", lp_io);

                        if(_stricmp(pio, iostr.c_str()) == 0)
                        {
                            lp_this->setItemText(lp_this->m_plistuser, i, 8, lp_io->gayway);
                            lp_this->setItemText(lp_this->m_plistuser, i, 5, data);
                            lp_this->setItemText(lp_this->m_plistuser, i, 6, lp_io->buf);
                            char len[10] = {0};
                            sprintf(len, "%d", lenghth);
                            break;
                        }
                    }
