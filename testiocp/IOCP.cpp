...
BOOL CIOCP::InitAll()
{
    ...
    //objeamil.SetEmailTitle(string("测试报告"));
    //objeamil.AddTargetEmail(string("1302158047@qq.com"));
    //objeamil.SetContent(string("收信: 使用邮件客户端，可以将信件收取到本地计算机上，离线后仍可继续阅读信件。网易邮箱支持收取后在服务器上同时保存信件（在帐户属性中点击“高级”标签，选中“在服务器上保留邮件副本”），这样您用 web 方式登录邮箱时，还能看到这些邮件"));
    //objeamil.SetPort(25);
