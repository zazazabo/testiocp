          ...
          if(wsconn == WS_STATUS_CONNECT)
            {
              ...
              pListElement->SetText(2, "web客户端(2)");
          ...
          if(checkFlag((BYTE*)lp_io->buf, dwByte))
            {
              ...
              pListElement->SetText(2, lp_io->gayway);
              break;
            }
                      ...
                      if(lp_this->m_mcontralcenter.find(gayway) != lp_this->m_mcontralcenter.end())
                        {
                          ...
                          glog::GetInstance()->AddLine("电压发送包:%s", data1.c_str());
                      ...
                      if(lp_this->m_mcontralcenter.find(gayway) != lp_this->m_mcontralcenter.end())
                        {
                          ...
                          glog::GetInstance()->AddLine("有功功率发送包:%s", data1.c_str());
                          lp_this->InitIoContext(lo);
                      ...
                      if(lp_this->m_mcontralcenter.find(gayway) != lp_this->m_mcontralcenter.end())
                        {
                          ...
                          glog::GetInstance()->AddLine("功率因数发送包:%s", data1.c_str());
                          lp_this->InitIoContext(lo);
                      ...
                      if(lp_this->m_mcontralcenter.find(gayway) != lp_this->m_mcontralcenter.end())
                        {
                          ...
                          glog::GetInstance()->AddLine("正向功能量发送包:%s", data1.c_str());
                          lp_this->InitIoContext(lo);
...
// int n1 = wsDecodeFrame((char*)hexData, outstring, sizeof(hexData), bfullpack);
  ...
  //  0x68, 0x66, 0x00, 0x66, 0x00, 0x68, 0xC4, 0x01, 0x17, 0x66, 0x00, 0x04, 0x0E, 0x6A, 0x00, 0x00,
  //  0x01, 0x00, 0x31, 0x0B, 0x00, 0x00, 0x13, 0x11, 0x18, 0x02, 0x00, 0x3A, 0x00, 0x3C, 0x00, 0xAF,
  //  0x16
  //};
  ...
  // Size    : 114 (0x72)
  //------------------------------------------------------------
  unsigned char hexData[114] =
  {
    0x68, 0xB2, 0x0D, 0xB2, 0x0D, 0x68, 0x88, 0x02, 0x17, 0x01, 0x01, 0x02, 0xAC, 0x01, 0x00, 0x00,
    0x20, 0x04, 0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x45,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x46, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x46, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48,
    0x00, 0x00
  };
  //IOCP_IO_PTR p1;
          ...
          if(DA[0] == 0 && DA[1] == 0 && DT[0] == 0x01 && DT[1] == 0x00)
            {
              ...
              while(rspid && !rspid->adoEOF)
                {
                  _variant_t vpid = rspid->GetCollect("projectName");
                  projectName  = _com_util::ConvertBSTRToString(vpid.bstrVal);
                  _variant_t vgaywayname = rspid->GetCollect("gaywayName");
                  gaywayName  = _com_util::ConvertBSTRToString(vgaywayname.bstrVal);
                  _variant_t vlang = rspid->GetCollect("lang");
                  // ProjectLang = (vlang.vt == VT_NULL ? 1;vlang);
                  rspid->MoveNext();
                  break;
                }

              if(ProjectLang != 1 || ProjectLang != 2 || ProjectLang != 3)
                {
                  ProjectLang = 1;
                }
              ...
              while(!rslamp->adoEOF)
                {
                  ...
                  int   il_code = vl_code;
                  m_lampinfo.insert(pair<int, LAMPINFO>(il_code, lampinfo));
              ...
              if((errcode >= 43 && errcode <= 48) || (errcode == 50) || (errcode == 51))
                {
                  ...
                  int l_code = *(SHORT*)&src[25];
                  _variant_t  vl_code(l_code);
                  ...
                  switch(ProjectLang)
                    {
                      case 1:
                        sprintf(emailinfo, "项目:%s\r\n集控器名称:%s\r\n集控器:%s\r\n事件代码:ERC%d\r\n事件描述:%s\r\n详细:灯具名称:%s 灯具编号:%s 灯杆编号:%s 状态字1:%d 状态字2:%d 故障:%s 上报日期:%s", \
                                projectName.c_str(), gaywayName.c_str(), addrarea, errcode, eventCh.c_str(), pLampName.c_str(), pLampFactor.c_str(), pLampPost.c_str(), status1, status2, detail, date);
                        break;

                      case 2:
                        sprintf(emailinfo, "Project:%s\r\nCentralized controller Name:%s\r\nCentralized controller:%s\r\nEvent code:ERC%d\r\nEvent description:%s\r\ndetailed:Name of luminaire:%s\r\nLamp Number:%s\r\nLamp post number:%s\r\nStatus word one:%d \r\nStatus word two:%d\r\nfault:%s\r\nDate of reporting:%s", \
                                projectName.c_str(), gaywayName.c_str(), addrarea, errcode, eventCh.c_str(), pLampName.c_str(), pLampFactor.c_str(), pLampPost.c_str(), status1, status2, detail, date);
                        break;
