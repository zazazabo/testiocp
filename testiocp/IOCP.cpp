                    ...
                    switch(ProjectLang)
                    {
                        case 1:
                            sprintf(emailinfo, "项目:%s\r\n集控器名称:%s\r\n集控器:%s\r\n事件代码:ERC%d\r\n事件描述:%s\r\n详细:%s\r\n灯具名称:%s\r\n灯具编号:%s\r\n灯杆编号:%s\r\n状态字1:%d \r\n状态字2:%d\r\n上报日期:%s", \
                                    projectName.c_str(), gaywayName.c_str(), addrarea, (int)errcode, eventCh, detail, pLampName.c_str(), pLampFactor.c_str(), pLampPost.c_str(), status1, status2, date);
                            break;

                        case 2:
                            sprintf(emailinfo, "Project:%s\r\nCentralized controller Name:%s\r\nCentralized controller:%s\r\nEvent code:ERC%d\r\nEvent description:%s\r\ndetailed:%s\r\nName of luminaire:%s\r\nLamp Number:%s\r\nLamp post number:%s\r\nStatus word one:%d \r\nStatus word two:%d\r\nDate of reporting:%s", \
                                    projectName.c_str(), gaywayName.c_str(), addrarea, (int)errcode, eventEn, pLampName, pLampName.c_str(), pLampFactor.c_str(), pLampPost.c_str(), status1, status2, date);
                            break;

                        case 3:
                            sprintf(emailinfo, "Проект:%s\r\nИмя шлюза:%s\r\nшлюз:%s\r\nКод события:ERC%d\r\nОписание события:%s\r\nдетализированный:%s\r\nНазвание лампы:%s\r\nНомер лампы:%s\r\nКоличество световых полюсов:%s\r\nСлово состояния 1:%d \r\nСлово состояния 2:%d\r\nДата отчета:%s", \
                                    projectName.c_str(), gaywayName.c_str(), addrarea, (int)errcode, eventE, pLampName, pLampName.c_str(), pLampFactor.c_str(), pLampPost.c_str(), status1, status2, date);
                            break;
                    }
                }
                else
...
BOOL CIOCP::dealRead(IOCP_IO_PTR & lp_io, IOCP_KEY_PTR & lp_key, DWORD dwBytes)
{
    ...
    string data = gstring::char2hex(lp_io->buf, datalen);
    datalen > 0 ? glog::GetInstance()->AddLine("包长度:%d 包数据:%s", datalen, data.c_str()) : glog::GetInstance()->AddLine("包长度:%d", datalen);
    towrite = data;
...
COMPLETEPACK:
    ...
    //    PostLog("集控器[%s] 包长度:%d 帧序号:%d 包数据:%s 通信指针:%p", addrarea, datalen, src[0xd] & 0x0f, datastr.c_str(), lp_io);
