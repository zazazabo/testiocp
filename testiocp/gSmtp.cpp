...
gSmtp::gSmtp()
{
  ...
  if((RecvBuf = new char[MAIL_BUFFER_SIZE]) == NULL)
    throw ECSmtp(ECSmtp::LACK_OF_MEMORY);

  if((SendBuf = new char[MAIL_BUFFER_SIZE]) == NULL)
    throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::AddAttachment(const char *Path)
{
  assert(Path);
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::AddRecipient(const char *email, const char *name)
{
  if(!email)
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::AddCCRecipient(const char *email, const char *name)
{
  if(!email)
    throw ECSmtp(ECSmtp::UNDEF_RECIPIENT_MAIL);
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::AddBCCRecipient(const char *email, const char *name)
{
  if(!email)
    throw ECSmtp(ECSmtp::UNDEF_RECIPIENT_MAIL);
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::AddMsgLine(const char* Text)
{
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::DelMsgLine(unsigned int Line)
{
...
// AUTHOR/DATE: JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::ModMsgLine(unsigned int Line, const char* Text)
{
  if(Text)
    {
      if(Line >= MsgBody.size())
  ...
  try
    {
      ...
      snprintf(SendBuf, MAIL_BUFFER_SIZE, "MAIL FROM:<%s>\r\n", m_sMailFrom.c_str());
      SendData(pEntry);
      ...
      for(i = 0; i < Recipients.size(); i++)
        {
          snprintf(SendBuf, MAIL_BUFFER_SIZE, "RCPT TO:<%s>\r\n", (Recipients.at(i).Mail).c_str());
          SendData(pEntry);
          ReceiveResponse(pEntry);
        }
      ...
      // DATA <CRLF>
      snprintf(SendBuf, MAIL_BUFFER_SIZE, "DATA\r\n");
      SendData(pEntry);
      ReceiveResponse(pEntry);
      ...
      if(GetMsgLines())
        {
          for(i = 0; i < GetMsgLines(); i++)
            {
              snprintf(SendBuf, MAIL_BUFFER_SIZE, "%s\r\n", GetMsgLineText(i));
              SendData(pEntry);
            }
        }
      else
      ...
      if(Attachments.size())
        {
          snprintf(SendBuf, MAIL_BUFFER_SIZE, "\r\n--%s--\r\n", BOUNDARY_TEXT);
          SendData(pEntry);
        }
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
bool gSmtp::ConnectRemoteServer(const char* szServer, const unsigned short nPort_/*=0*/,
          ...
          if(IsKeywordSupported(RecvBuf, "LOGIN") == true)
            {
              ...
              snprintf(SendBuf, MAIL_BUFFER_SIZE, "AUTH LOGIN\r\n");
              SendData(pEntry);
              ReceiveResponse(pEntry);
              // send login:
          ...
          else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
            {
              ...
              snprintf(SendBuf, MAIL_BUFFER_SIZE, "%s^%s^%s", m_sLogin.c_str(), m_sLogin.c_str(), m_sPassword.c_str());
              ...
              delete[] ustrLogin;
              snprintf(SendBuf, MAIL_BUFFER_SIZE, "AUTH PLAIN %s\r\n", encoded_login.c_str());
              SendData(pEntry);
              ReceiveResponse(pEntry);
            }
          else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
            {
              ...
              snprintf(SendBuf, MAIL_BUFFER_SIZE, "AUTH CRAM-MD5\r\n");
              SendData(pEntry);
              ReceiveResponse(pEntry);
          ...
          else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
            {
              ...
              snprintf(SendBuf, MAIL_BUFFER_SIZE, "AUTH DIGEST-MD5\r\n");
              SendData(pEntry);
              ReceiveResponse(pEntry);
              std::string encoded_challenge = RecvBuf;
              encoded_challenge = encoded_challenge.substr(4);
              ...
              //send the response
              if(strstr(RecvBuf, "charset") >= 0) snprintf(SendBuf, MAIL_BUFFER_SIZE, "charset=utf-8,username=\"%s\"", m_sLogin.c_str());
              else snprintf(SendBuf, MAIL_BUFFER_SIZE, "username=\"%s\"", m_sLogin.c_str());

              if(!realm.empty())
                {
                  snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",realm=\"%s\"", realm.c_str());
                  strcat(SendBuf, RecvBuf);
                }

              snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",nonce=\"%s\"", nonce.c_str());
              strcat(SendBuf, RecvBuf);
              snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",nc=%s", nc);
              strcat(SendBuf, RecvBuf);
              snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",cnonce=\"%s\"", cnonce);
              strcat(SendBuf, RecvBuf);
              snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",digest-uri=\"%s\"", uri.c_str());
              strcat(SendBuf, RecvBuf);
              snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",response=%s", decoded_challenge.c_str());
              strcat(SendBuf, RecvBuf);
              snprintf(RecvBuf, MAIL_BUFFER_SIZE, ",qop=%s", qop.c_str());
              strcat(SendBuf, RecvBuf);
...
////////////////////////////////////////////////////////////////////////////////
void gSmtp::DisconnectRemoteServer()
{
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void gSmtp::FormatHeader(char* header)
{
  ...
  snprintf(header, MAIL_BUFFER_SIZE, "Date: %d %s %d %d:%d:%d\r\n", timeinfo->tm_mday,
...
////////////////////////////////////////////////////////////////////////////////
void gSmtp::ReceiveData(Command_Entry* pEntry)
{
  if(m_ssl != NULL)
    {
  ...
  if(FD_ISSET(hSocket, &fdread))
    {
      res = recv(hSocket, RecvBuf, MAIL_BUFFER_SIZE, 0);
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SendData(Command_Entry* pEntry)
{
  if(m_ssl != NULL)
    {
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* gSmtp::GetLocalHostName()
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
unsigned int gSmtp::GetRecipientCount() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
unsigned int gSmtp::GetBCCRecipientCount() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
unsigned int gSmtp::GetCCRecipientCount() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* gSmtp::GetReplyTo() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* gSmtp::GetMailFrom() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* gSmtp::GetSenderName() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* gSmtp::GetSubject() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* gSmtp::GetXMailer() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
CSmptXPriority gSmtp::GetXPriority() const
{
  return m_iXPriority;
}

const char* gSmtp::GetMsgLineText(unsigned int Line) const
{
  if(Line >= MsgBody.size())
    throw ECSmtp(ECSmtp::OUT_OF_MSG_RANGE);
...
unsigned int gSmtp::GetMsgLines() const
{
  return MsgBody.size();
}
...
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetCharSet(const char *sCharSet)
{
  m_sCharSet = sCharSet;
}
...
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetLocalHostName(const char *sLocalHostName)
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetXPriority(CSmptXPriority priority)
{
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetReplyTo(const char *ReplyTo)
{
  m_sReplyTo = ReplyTo;
}
...
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetReadReceipt(bool requestReceipt/*=true*/)
{
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetSenderMail(const char *EMail)
{
  m_sMailFrom = EMail;
}
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetSenderName(const char *Name)
{
  m_sNameFrom = Name;
}
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetSubject(const char *Subject)
{
  m_sSubject = Subject;
}
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetXMailer(const char *XMailer)
{
  m_sXMailer = XMailer;
}
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetLogin(const char *Login)
{
  m_sLogin = Login;
}
...
//              JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetPassword(const char *Password)
{
...
//              JO 2010-0708
////////////////////////////////////////////////////////////////////////////////
void gSmtp::SetSMTPServer(const char* SrvName, const unsigned short SrvPort, bool authenticate)
{
...
void gSmtp::SayQuit()
{
  ...
  // QUIT <CRLF>
  snprintf(SendBuf, MAIL_BUFFER_SIZE, "QUIT\r\n");
  m_bConnected = false;
  SendData(pEntry);
  ReceiveResponse(pEntry);
}
...
void gSmtp::StartTls()
{
  ...
  snprintf(SendBuf, MAIL_BUFFER_SIZE, "STARTTLS\r\n");
  SendData(pEntry);
  ReceiveResponse(pEntry);
  OpenSSLConnect();
}

void gSmtp::ReceiveData_SSL(SSL* ssl, Command_Entry* pEntry)
{
  int res = 0;
  int offset = 0;
  fd_set fdread;
          ...
          while(1)
            {
              ...
              if(ssl_err == SSL_ERROR_NONE)
                {
                  if(offset + res > MAIL_BUFFER_SIZE - 1)
                    {
                      FD_ZERO(&fdread);
                      FD_ZERO(&fdwrite);
                      throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
...
void gSmtp::ReceiveResponse(Command_Entry* pEntry)
{
  std::string line;
  int reply_code = 0;
  bool bFinish = false;
  ...
  snprintf(RecvBuf, MAIL_BUFFER_SIZE, line.c_str());
...
void gSmtp::SendData_SSL(SSL* ssl, Command_Entry* pEntry)
{
...
void gSmtp::setSendFlag(BOOL bsend)
{
  m_bsendflag = bsend;
}
